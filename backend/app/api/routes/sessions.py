...
def _pick_available_fl_port(base_port: int, host: str="0.0.0.0") -> int:
    ...
    connect() succeeds only if something is already listening â†’ port busy.
    connect() fails (ECONNREFUSED) â†’ port is free.
    """
                ...
                def _spawn_clients(participants_list, lead_data_dir, port, shape, arch):
                    ...
                    try:
                        ...
                        _el2.run_until_complete(event_bus.publish("local_clients_spawned", {
                            ...
                            "count": client_count,
                            "fl_port": port,
                            "message": f"Launching {client_count} local FL client(s) â†’ 127.0.0.1:{port}",
                        }))
                        _el2.close()
                    except Exception:
                        pass
