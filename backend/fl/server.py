...
def _persist_round_progress(session_key: str, server_round: int):
    ...
    different thread with its own loop â†’ 'Future attached to a different loop'.
...
class ValidationStrategy(FedAvg):
    ...
    def configure_fit(
        ...
        for client_proxy, fit_ins in client_instructions:
            ...
            logger.info(
                f"[ValidationStrategy] Client validated âœ“ shape={client_shape} num_classes={client_num_classes}"
            )
